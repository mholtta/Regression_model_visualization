...
class Data (val file: String) {
  ...
  private def loadXLSX(file: String) = {
    ...
    // Try and finally to close file even in case of exceptions
    try {
      ...
      val allContent = allContentOption.map(_.flatten)
      
      
      
      // Getting header
      val header = allContent.head.map( x => formatter.formatCellValue(x))
      
      // Getting data, toDouble may create an exception, 
      val content = allContent.drop(1).map(_.map(x => x.getNumericCellValue))
  ...
  private def loadCSV(file: String) = {
    ...
    // Try and finally to close file even case of exception
    try {
      val loader = source.getLines()
